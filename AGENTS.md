...
| `gmail-reply-scan.mjs` | Opt-in automatic Gmail scan for employer replies (interview/offer/rejection/etc.) — appends only messages matched to an active tracker row into `data/reply-candidates.json`, unclassified; never classifies, never touches the tracker. Config: `config/plugins.yml` `gmail-reply-scan` block |
