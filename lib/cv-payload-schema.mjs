 ...
 * the LaTeX template's education entry really is {institution, degree, dates,
 * coursework}, while the HTML template's is {title, org, year, description}.
 * Keeping both tables side by side here makes the divergence explicit and lets
 * each builder reject the other's vocabulary by name instead of silently
 * dropping it.
 */
...
export const ENTRY_FIELD_SPECS = {
  html: {
    // `role`/`dates`/`bullets` describe a single-title entry (unchanged legacy
    // shape). `roles` is an additive alternative (#warnick-template): an array
    // of { role, dates, period, bullets, location } sub-entries rendered under
    // ONE company header instead of repeating the company name once per title
    // change (e.g. three promotions at the same employer). `role` is
    // deliberately NOT required here — an entry may satisfy either shape — so
    // the actual "does this render anything" gate lives in buildExperience()'s
    // own filter, matching the pattern skills.items already uses via `rules`.
    experience: {
      required: ['company'],
      known: ['company', 'role', 'location', 'dates', 'period', 'bullets', 'roles'],
    },
    projects: {
      required: ['name'],
    ...
    education: {
      required: ['title'],
      known: ['title', 'org', 'year', 'description'],
    },
